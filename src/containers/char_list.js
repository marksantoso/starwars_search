    ...
    render() {
        ...
        let charsWithScore = chars.map((char) => {
            if (Object.keys(this.props.scores).length > 0) {
                Object.entries(this.props.scores).forEach(([key, value]) => {
                    if (key === char.name) {
                        char = {
                            ...char,
                            ...this.props.scores[key]
                        };
                    }
                });
                return char;
        ...
        return (
            <InfiniteScroll next={this._loadMore.bind(this)} hasMore={this.state.moreItems} endMessage={complete} scrollThreshold={0.9} loader={loader} style={{
                overflow: 'hidden'
            }}>
                {charItems}
            </InfiniteScroll>
