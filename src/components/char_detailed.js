...
class CharDetailedModal extends Component {
    ...
    render() {
        ...
        const charImage = (this.props.char.thumbnailUrl !== null)
            ? this.props.char.thumbnailUrl
            : noImage;
        ...
        const charItem = Object.keys(charDetail).map((prop) => {
            if (typeof charDetail[prop] == 'object') {
                const sublistitems = Object.keys(charDetail[prop]).map((subprop) => {
                ...
                return (
                    <li key={prop}>
                        <span className="prop">{prop}: {charDetail[prop]}</span>
                    </li>
                );
            }
        });
